...
if admin_ids:
    with sqlite3.connect(path_db) as connection:
        ...
        for item in admin_ids:
            print(item)
            cursor.execute(
                "INSERT OR IGNORE INTO Users(tg_id) VALUES(?)", (item,)
            )  # записываем админа в таблицу Users, если еще не записан
            cursor.execute(
                "SELECT id FROM Users WHERE tg_id = ?", (item,)
            )  # Извлекаем его id
            (adm,) = cursor.fetchone()
            print("admin_id = ", adm)
    ...
    with sqlite3.connect(path_db) as connection:
        ...
        for ad in adm_ids:
            ...
            cursor.execute(
                ...
                (ad, club_id),
            )  # Добвляем админов в список участников группы, если еще не добавлены
            cursor.execute(
                "SELECT id FROM Members WHERE user_id = ? AND club_id = ?",
                (ad, club_id),
            )  # извлекаем memder_id админа
            (m_id,) = cursor.fetchone()
            cursor.execute(
                "INSERT OR IGNORE INTO Status(member_id, status) VALUES(?,?)",
                (m_id, "owner"),
            )  # присваем статус владельца в соотвествующей группе
            cursor.execute(
                "INSERT OR IGNORE INTO Status(member_id, status) VALUES(?,?)",
                (m_id, "member"),
            )  # присваем статус участника в соотвествующей группе
