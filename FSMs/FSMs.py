...
class FSMNewStatus(StatesGroup):
    ...
    fill_new_status_confirm = (
        State()
    )  # Состояние ожидания подтверждения выбранного статуса
...
class FSMRereg(StatesGroup):
    ...
    fill_no_reg = (
        State()
    )  # Состояние обновления данных - без регистрации (не вызывается выбор регистраторов)
...
class FSMNewVariant(StatesGroup):
    ...
    fill_more_variant = (
        State()
    )  # Состояние ожидания выбора добавлять еще вариант или нет
