...
async def voting_create(
    ...
    if len(title) > 40:
        flag = False
        answ_str = (
            "Название не должно быть длиннее 40 символов."
        ...
        async with AsyncDatabase(path_db) as cursor:
            try:
                ...
                if title not in existing_titles:
                    logger.info(
                        f"Добавление нового голосования: club_id={club_id}, creator={creator}, title={title}"
                    )
                    await cursor.execute(
                        """
                    ...
                    answ_str = (
                        "Уже есть идущее голосование с таким названием."
                        " Придумайте другое название."
                    )
                    flag = False
            except aiosqlite.Error as e:
                logger.error(
                    f"Ошибка при создании нового голосования: {e}\n{traceback.format_exc()}"
                )
                raise
...
async def get_voting_info(voting_id):
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            logger.error(
                f"Ошибка при извлечении информации о группе voting_id={voting_id}: {e}\n{traceback.format_exc()}"
            )
            raise
        ...
        async with AsyncDatabase(path_db) as cursor:
            try:
                ...
                if status == "add_variants":
                    logger.info(
                        f"Добавление нового варианта: voting_id={voting_id}, author={author}, title={title}"
                    )
                ...
                logger.error(
                    f"Ошибка при добавлении нового варианта: {e}\n{traceback.format_exc()}"
                )
                raise
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            if status != "add_variants":
                flag = False
                response = (
                    "Это голосование не находится в стадии добавления вариантов, "
                    "вы не можете его запустить."
                )
            elif count_of_var < 2:
                flag = False
                response = (
                    "У этого голосования меньше двух вариантов, вы не можете его запустить. "
                ...
                logger.info(
                    f"Голосование {voting_id} запущено пользователем {starter}."
                )
                flag = True
            ...
            logger.error(
                f"Ошибка при старте голосования: {e}\n{traceback.format_exc()}"
            )
            raise
...
async def extract_voting_id(variant_id):
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            logger.error(
                f"Ошибка при получении ID голосования: {e}\n{traceback.format_exc()}"
            )
            raise
...
async def extract_group_id(voting_id):
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            logger.error(
                f"Ошибка при получении ID группы: {e}\n{traceback.format_exc()}"
            )
            raise
...
async def past_choise(member_id, voting_id):
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            logger.info(
                f"Результат выборки для member_id={member_id}, voting_id={voting_id}: {result}"
            )
            return result
        except aiosqlite.Error as e:
            logger.error(
                f"Ошибка при проверке прошлых выборов: {e}\n{traceback.format_exc()}"
            )
            raise
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            if result:
                (amount,) = result
                logger.info(
                    f"Количество голосов, доверенных представителю {proxy}: {amount}"
                )
                return int(amount)
            else:
            ...
            logger.error(
                f"Ошибка при подсчете числа доверенных голосов: {e}\n{traceback.format_exc()}"
            )
            raise
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            if result:
                (amount,) = result
                logger.info(
                    f"Количество участников с правом голоса в группе {club_id}: {amount}"
                )
                return int(amount)
            else:
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            if result:
                (amount,) = result
                logger.info(
                    f"Количество прямых голосов за variant_id={variant_id}: {amount}"
                )
                return int(amount)
            else:
            ...
            logger.error(
                f"Ошибка при подсчете прямых голосов: {e}\n{traceback.format_exc()}"
            )
            raise
...
async def count_directly_empty_votes(variant_id):
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            logger.info(
                f"Количество прямых голосов без права голоса за variant_id={variant_id}: {amount}"
            )
            return amount
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            await cursor.execute(
                ...
                SELECT COUNT(*)
                FROM Members m
                -- Проверяем, что участник имеет статус 'votist'
                INNER JOIN Status s ON m.id = s.member_id AND s.status = 'votist'
                -- Проверяем, что proxy заполнен
                WHERE m.proxy IS NOT NULL
                -- Проверяем, что представитель имеет статус 'proxy'
                AND EXISTS (
                    SELECT 1
                    FROM Status sp
                    WHERE sp.member_id = m.proxy
                      AND sp.status = 'proxy'
                )
                -- Проверяем, что представитель проголосовал за нужный вариант
                AND EXISTS (
                    SELECT 1
                ...
                AND NOT EXISTS (
                    SELECT 1
                    FROM Elections e_self
                    INNER JOIN Variants v ON e_self.variant_id = v.id
                    WHERE e_self.member_id = m.id
                      AND v.voting_id = (
                          SELECT voting_id
                          FROM Variants
                          WHERE id = ?
                      )
                      AND e_self.status = 'valid'
                )
                """,
...
async def lose_variant(losers, voting_id=None, result=None, stager=None):
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            for item in losers:
                if not result:
                    dir_votes = await count_directly_votes(
                        item
                    )  # решающие голоса, поданные за вариант напрямую
                    proxy_votes = await count_proxy_votes(
                        item
                    )  # решающие голоса, поданные через представителя
                    empty_votes = await count_directly_empty_votes(
                        item
                    )  # нерешающие голоса
                else:
            ...
            logger.error(
                f"Ошибка при записи проигравшего варианта: {e}\n{traceback.format_exc()}"
            )
            raise
...
async def win_variant(winner_id, voting_id=None, result=None, stager=None):
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            logger.error(
                f"Ошибка при записи про промежуточного этапа голосования: {e}\n{traceback.format_exc()}"
            )
            raise
...
async def voting_stage(voting_id, club_id=None, stager=None) -> Dict[str, Any]:
    ...
    for item in variants:
        dir_votes = (
            await count_directly_votes(item["id"]) or 0
        )  # Если None, используем 0
        proxy_votes = (
            await count_proxy_votes(item["id"]) or 0
        )  # Если None, используем 0
        empty_votes = (
            await count_directly_empty_votes(item["id"]) or 0
        )  # Если None, используем 0
        res[item["id"]] = (
            dir_votes + proxy_votes,
            dir_votes,
            empty_votes,
            item["title"],
        )
        ...
        async with AsyncDatabase(path_db) as cursor:
            try:
                if losers:
                    ...
                    los_titles = [
                        res[item][3] for item in losers
                    ]  # Список названий проигравших вариантов
                    losers_str = "\n".join(los_titles)
                    flag = True
                ...
                if len(winners) == 1:
                    ...
                    logger.info(
                        f"Голосование завершено. Победивший вариант: {winner_id}, Проигравшие варианты: {losers}"
                    )
                    flag = True
                ...
                logger.error(
                    f"Ошибка при завершении промежуточного этапа голосования: {e}\n{traceback.format_exc()}"
                )
                flag = False
...
async def voting_final(voting_id, finaler=None) -> Dict[str, Any]:
    ...
    if not variants:
        ...
        logger.info(
            f"У voting_id={voting_id} только два действительных варианта. Переводим их в финал."
        )
        flag = True
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            logger.error(
                f"Ошибка при создании финального этапа голосования: {e}\n{traceback.format_exc()}"
            )
            flag = False
...
async def voting_complete(voting_id, finisher=None) -> Dict[str, Any]:
    ...
    winner_title = next(
        (item["title"] for item in variants if item["id"] == winner_id), None
    )
    ...
    if (winner_res[0] * 2 < s_votist) and voting_info.get(
        "voting_status"
    ) != "confirmation":
        ...
        try:
            await win_variant(
                winner_id, voting_id=voting_id, result=res[winner_id], stager=finisher
            )
            logger.info(
                f"Победивший вариант: {winner_id}, Проигравшие варианты: {losers}"
            )
            text = f"""Победил вариант {winner_title}.\n
            Его результат:\n
            Всего голосов "за": {winner_res[0]}\n
            ...
            logger.error(
                f"Ошибка при завершении голосования: {e}\n{traceback.format_exc()}"
            )
            success = False
...
async def confirmation_of_voting_results(voting_id, winner_id):
    ...
    async with AsyncDatabase(path_db) as cursor:
        ...
        # !!!!!!!!!!!!!!!!!!
            ...
            logger.error(
                f"Ошибка при запуске утверждения итогов голосования: {e}\n{traceback.format_exc()}"
            )
            raise
    ...
    async with AsyncDatabase(path_db) as cursor:
        try:
            ...
            if result:
                ...
                logger.info(
                    f"Статус голосования voting_id={voting_id}: {voting_status}"
                )
            ...
            logger.error(
                f"Ошибка при получении статуса голосования: {e}\n{traceback.format_exc()}"
            )
            raise
