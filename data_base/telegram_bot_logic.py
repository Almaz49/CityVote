...
async def new_variant_tg(voting_id, creator_tg_id, title, text=None):
    ...
    if not author:
        logger.warning(
            f"Пользователь с tg_id={creator_tg_id} не является участником группы"
        )
        return False, "Вы не являетесь участником группы"

    result = await variant_create(voting_id, author, title, text=text)
    logger.info(
        f"Добавлен новый вариант для голосования voting_id={voting_id} от tg_id={creator_tg_id}: {result}"
    )
    return result
...
async def list_of_members_tg(status):
    try:
        ...
        logger.info(
            f"Извлечены участники для club_id={club_id} с status={status}: {members}"
        )
        return members
    except Exception as e:
...
async def trust_tg(tg_id, proxy_tg_id):
    try:
        ...
        logger.info(
            f"Пользователь  с tg_id {tg_id} выбрал представителем учатника с tg_id {proxy_tg_id}"
        )
        return True, result
    except aiosqlite.Error as db_error:
        logger.error(
            f"Ошибка базы данных при выборе представителя пользователем {tg_id}: {db_error}"
        )
        return False, f"Ошибка базы данных: {str(db_error)}"
    except ValueError as value_error:
        logger.error(
            f"Ошибка значения при выборе представителя пользователем {tg_id}: {value_error}"
        )
        return False, f"Ошибка значения: {str(value_error)}"
    except Exception as e:
        logger.error(
            f"Произошла неизвестная ошибка при выборе представителя пользователем {tg_id}: {e}"
        )
...
async def election_tg(tg_id, variant_id):
    try:
        ...
        if member_id:
            ...
            logger.info(
                f"Выбран вариант variant_id={variant_id} участником с tg_id={tg_id}: {result}"
            )
            return True, result
        else:
...
async def voting_start_tg(voting_id, starter_tg_id=None):
    try:
        if starter_tg_id:
            starter = await extract_member_id(
                club_id, await extract_user_id(starter_tg_id)
            )
...
async def voting_stage_tg(voting_id, stager_tg_id=None):
    try:
        if stager_tg_id:
            stager = await extract_member_id(
                club_id, await extract_user_id(stager_tg_id)
            )
        ...
        logger.info(
            f"Промежуточный этап голосования voting_id={voting_id} завершен: {result}"
        )
        return result
    except Exception as e:
...
async def voting_final_tg(voting_id, finaler_tg_id=None):
    try:
        if finaler_tg_id:
            finaler = await extract_member_id(
                club_id, await extract_user_id(finaler_tg_id)
            )
            if not finaler:
                logger.warning(
                    f"Пользователь с tg_id={finaler_tg_id} не является участником группы"
                )
                return False, "Вы не являетесь участником группы"
        ...
        logger.info(
            f"Создан финальный этап голосования voting_id={voting_id}: {result}"
        )
        return result
    except Exception as e:
...
async def voting_complete_tg(voting_id, finisher_tg_id=None):
    try:
        if finisher_tg_id:
            finisher = await extract_member_id(
                club_id, await extract_user_id(finisher_tg_id)
            )
            if not finisher:
                logger.warning(
                    f"Пользователь с tg_id={finisher_tg_id} не является участником группы"
                )
                return False, "Вы не являетесь участником группы"
...
async def update_address(tg_id, city, street, house):
    try:
        ...
        result = await update_user_data(
            user_id=user_id, city=city, street=street, house=house
        )
        if result:
